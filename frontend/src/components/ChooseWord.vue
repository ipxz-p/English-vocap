<template>
    <div class="m-5" id="app">
        <div class="flex flex-col gap-4">
            <span class="animate-headShake text-3xl font-extrabold text-mypink-400">Choose the word set !</span>
        </div>
        <div class="flex flex-col gap-4">
            <span class="animate-flash text-2xl font-extrabold text-mypink-400 mt-2">My word set</span>
        </div>

        <!-- <router-link to="/chooseword/play">
            <div class="card bg-red-500 w-24 m-10">
                Click card ja
            </div>
        </router-link> -->
        <!-- card word set container -->
        <div v-if="myWordSet.length > 0" class="grid gap-4 mt-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            <CardWordSet v-for="item in myWordSet" :key="item.wordSet_id" :id="item.wordSet_id" :username="item.username"
                :name="item.name" :description="item.description" :total_word="item.total_word"
                :total_play="item.total_play" :toName="'Play'" />
        </div>
        <div v-else class="mt-4">
            <div class="flex flex-col justify-center ml-5 items-center w-32">
                <div class="w-24">
                <!-- <img src="../assets/img/Easter balloon _Monochromatic.svg" alt=""> -->
             </div>
             <div class="font-medium text-lg text-gray-500">No Word Set</div>
            </div>
             
        </div>

        <div class="flex flex-col gap-4">
            <span class="animate-flash text-2xl font-extrabold text-mypink-400 mt-2">Other word set</span>
        </div>

        <!-- <router-link to="/chooseword/play">
            <div class="card bg-red-500 w-24 m-10">Click card ja</div>
        </router-link> -->
        ...
        <div v-else class="mt-4">no data</div>
        <!-- Popup edit wordset -->
        <Popup type="editWordSet" v-if="this.$store.state.showPopup &&
            ...
            <div @click.stop="">
                <div class="font-medium text-2xl flex justify-between items-center">
                    <p>Edit word set</p>
                    <svg @click="setPopup('editWordSet')" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 cursor-pointer"
                ...
                <div class="mt-4">
                    ...
                    <div class="px-3 mt-1 border-2 border-gray-300 w-full rounded-xl h-10">
                        <input v-model="title" class="outline-0 w-full h-full" type="text" name=""
                ...
                <div class="mt-2">
                    ...
                    <div class="px-3 mt-1 border-2 border-gray-300 w-full rounded-xl h-10">
                        <input v-model="description" class="outline-0 w-full h-full" type="text" name=""
                            placeholder="Add description" />
                    </div>
                </div>
                <button @click="editWordSet()" class="w-full bg-mypink-300 text-white py-2 mt-4 rounded-xl" type="submit">
                    Edit
                </button>
            </div>
        </Popup>
        <!-- Popup comfirm before delete wordset -->
        <Popup type="deleteWordSet" v-if="this.$store.state.showPopup &&
            ...
            <div @click.stop="">
                ...
                <div class="flex">
                    <button @click="setPopup('deleteWordSet')" class="w-full py-2 mt-4 rounded-xl" type="submit">
                        Cancel
                    </button>
                    <button @click="deleteWordSet()"
...
export default {
    ...
    methods: {
        ...
        async editWordSet() {
            await axios
                ...
                .then(async (res) => {
                    await axios
                        .post("/wordSet/getWordSetByUsername", {
                            username: this.username,
                        })
                        .then((res) => {
                            (this.myWordSet = res.data),
                                (this.title = ""),
                                (this.description = "");
                            this.setPopup();
                        })
            ...
            confirmButtonText: 'ok'
                    ...
                    await axios
                        .post("/wordSet/getWordSetNotThisUsername", {
                            username: this.username,
                        })
                        .then((res) => {
    ...
    async created() { //ก่อนแสดงหน้าเว็บให้ทำอะไร
        if (!this.$store.getters.userName.username) {
            await axios.get("/wordSet/getAllWordSet").then((res) => {
                this.notMyWordSet = res.data;
            });
        } else {
            await axios
