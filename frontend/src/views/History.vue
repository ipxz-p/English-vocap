<template>
  <div class="m-5">
    <p class="text-3xl font-semibold">History</p>
    <!-- card container -->
    <div
    ...
    >
      <div
        v-if="words.length > 0"
        v-for="item in words"
        class="border-2 border-yellow-400 rounded-lg px-4 py-3 bg-yellow-100"
      >
        <p class="font-medium text-lg">{{ item.name }}</p>
        <p class="text-gray-500 truncate">{{ item.description }}</p>
        <div class="text-gray-500 truncate flex items-center mt-4">
